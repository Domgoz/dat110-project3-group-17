...
public void fixFingerTable() {
	...
	try {
		...
		BigInteger modulus = no.hvl.dat110.util.Hash.addressSize();
		...
		for (int i = 0; i < m; i++) {
			...
			BigInteger k = n.add(twoPowI).mod(modulus);

			NodeInterface succnode = chordnode.findSuccessor(k);

			if (succnode != null) {
				chordnode.getFingerTable().add(succnode);
			}
		}

		logger.info("FingerTable fixed for " + chordnode.getNodeName() +
				" | entries=" + chordnode.getFingerTable().size());

	} catch (RemoteException e) {
		logger.error("fixFingerTable() failed: " + e.getMessage());
	}
}
