...
public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
    ...
    WANTS_TO_ENTER_CS = true;
        ...
        List<Message> activenodes = new ArrayList<>(node.activenodesforfile);

        multicastMessage(message, activenodes);

        int numvoters = activenodes.size();
        boolean allReturned = areAllMessagesReturned(numvoters);
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for (Message nodes : activenodes) {
			int port = nodes.getPort();
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				if (sendClock < myClock) {
					senderWins = true;
				} else if (sendClock > myClock) {
					senderWins = false;
				} else {
					BigInteger senderID = message.getNodeID();
					BigInteger myID = node.getNodeID();
					if (senderID.compareTo(myID) < 0) {
						senderWins = true;
					} else {
						senderWins = false;
					}
